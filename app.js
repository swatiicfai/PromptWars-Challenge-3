    ...
    questions: [
      {
        ...
        correctIndex: 1,
        explanation:
          "Electricity and heat production accounts for roughly 25% of global greenhouse gas emissions, followed closely by agriculture/land use (24%) and industry (21%).",
      },
      {
        ...
        correctIndex: 1,
        explanation:
          "Methane is a potent greenhouse gas. While it has a shorter lifetime in the atmosphere, it is 28-36 times more effective at trapping heat than CO2 over 100 years.",
      },
      {
        q: "On average, how much carbon dioxide (CO2) does a mature, healthy tree absorb per year?",
        options: ["About 2 kg", "About 22 kg", "About 150 kg", "About 500 kg"],
        correctIndex: 1,
        explanation:
          "A mature tree absorbs roughly 22 kg (48 lbs) of carbon dioxide per year, making forestry conservation a powerful climate tool.",
      },
      {
        ...
        correctIndex: 2,
        explanation:
          "Plant proteins like lentils and beans produce up to 50 times less emissions than beef, and are significantly lower than poultry or fish.",
      },
      {
        ...
        correctIndex: 1,
        explanation:
          "Embodied carbon is the sum of all greenhouse gas emissions that occur during extraction of raw materials, processing, transportation, and building assembly.",
      },
    ],
      ...
      if (parsed.challenges && Array.isArray(parsed.challenges)) {
        parsed.challenges.forEach((c) => {
          ...
          if (localC) {
            ...
            localC.progress =
              c.progress !== undefined ? c.progress : localC.progress;
          }
        });
      }
    } catch (e) {
...
function initTheme() {
  ...
  const lightThemeActive =
    localStorage.getItem("terralife_light_theme") === "true";
  if (lightThemeActive) {
...
function gainXP(amount) {
  ...
  if (levelsGained > 0) {
    ...
    triggerNotification(
      `🎉 Congratulations! You reached Level ${appState.user.level} Eco-Hero! Keep it up!`,
      "award",
    );
...
function updateXPDisplay() {
  ...
  if (xpEl)
    xpEl.textContent = `${appState.user.xp} / ${appState.user.levelUpThreshold} XP`;
  if (barEl) {
...
function initCalculator() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    appState.history[appState.history.length - 1].emissions = parseFloat(
      calcRes.total.toFixed(1),
    );
    ...
    if (localStorage.getItem("terralife_calculator_rewarded") !== "true") {
      ...
      triggerNotification(
        "🎉 Baseline Footprint calculated! Earned 100 XP baseline reward.",
        "sparkles",
      );
    } else {
      triggerNotification(
        "🔄 Baseline Footprint updated successfully!",
        "check-circle",
      );
    }
    ...
    checkChallengeCriteria(
      "solar-sovereign",
      appState.calculator.inputs.solarPercent,
    );
...
function calculateFootprint(inputs) {
  ...
  else if (inputs.transMode === "walk-bike") transFactor = 0.0;
  ...
  if (inputs.dietType === "meat-heavy") diet = 2.9;
  else if (inputs.dietType === "meat-moderate") diet = 1.8;
  else if (inputs.dietType === "vegetarian") diet = 1.1;
  else if (inputs.dietType === "vegan") diet = 0.6;

  // Sourcing organic discounts: rarely = 0, sometimes = -0.1t, mostly = -0.25t
  if (inputs.foodLocal === "sometimes") diet -= 0.1;
  else if (inputs.foodLocal === "mostly") diet -= 0.25;

  // 4. LIFESTYLE & WASTE
  // Shopping habits: minimalist = 0.5, average = 1.2, heavy = 2.2
  if (inputs.shoppingHabits === "minimalist") waste = 0.5;
  else if (inputs.shoppingHabits === "average") waste = 1.2;
  else if (inputs.shoppingHabits === "heavy") waste = 2.2;
...
function recalculateLiveEstimate() {
  const transMode =
    document.querySelector('input[name="trans-mode"]:checked')?.value ||
    "car-petrol";
  const carDistance =
    parseFloat(document.getElementById("car-dist-input")?.value) || 0;
  ...
  const flightsShort =
    parseInt(document.getElementById("flights-short")?.value) || 0;
  const flightsLong =
    parseInt(document.getElementById("flights-long")?.value) || 0;
  const energyKwh =
    parseFloat(document.getElementById("energy-bill")?.value) || 0;
  const heatingSource =
    document.querySelector('input[name="heating-source"]:checked')?.value ||
    "natural-gas";
  const solarPercent =
    parseFloat(document.getElementById("solar-pct")?.value) || 0;
  const dietType =
    document.querySelector('input[name="diet-type"]:checked')?.value ||
    "meat-moderate";
  const foodLocal =
    document.querySelector('select[name="food-local"]')?.value || "rarely";
  const recycleHabits =
    document.querySelector('input[name="recycle-habits"]:checked')?.value ||
    "yes";
  const shoppingHabits =
    document.querySelector('input[name="shopping-habits"]:checked')?.value ||
    "average";
  ...
  if (comparisonText) {
    if (res.total <= 2.0) {
      comparisonText.innerHTML =
        "✨ <strong>Excellent!</strong> You are at the target climate neutrality goal (2.0 tonnes/year).";
    } else if (res.total <= 5.0) {
      comparisonText.innerHTML =
        "👍 <strong>Good score:</strong> You are well below the national average of 10.0 tonnes/year.";
    } else if (res.total <= 9.0) {
      comparisonText.innerHTML =
        "⚠️ <strong>Moderate:</strong> You are close to the average citizen's output. Look at transport/energy tabs to trim down.";
    } else {
      comparisonText.innerHTML =
        "🚨 <strong>High footprint:</strong> Swapping some car trips or switching electricity sources will make a massive impact!";
    }
  }
}
...
function renderDashboard() {
  ...
  const monthlyFootprintEl = document.getElementById(
    "dashboard-monthly-footprint",
  );
  ...
  const savedEquivalentEl = document.getElementById(
    "dashboard-saved-equivalent",
  );
  ...
  if (totalSavedEl)
    totalSavedKg === 0
      ? (totalSavedEl.textContent = "0.0 kg")
      : (totalSavedEl.textContent = `${totalSavedKg.toFixed(1)} kg`);
  ...
  if (baselineCalculated) {
    ...
    const currentActiveSavingsKg =
      parseFloat(document.getElementById("today-co2-saved")?.textContent) || 0;
    const netMonthlyKg = Math.max(
      0,
      baselineMonthlyKg - currentActiveSavingsKg,
    );

    if (monthlyFootprintEl)
      monthlyFootprintEl.textContent = Math.round(netMonthlyKg);
    ...
    if (gaugeStatusEl)
      gaugeStatusEl.textContent = "Click 'Calculator' to set baseline";
    if (comparisonPctEl) comparisonPctEl.textContent = "0%";
    if (comparisonSubEl)
      comparisonSubEl.textContent = "Baseline not calculated yet";
...
function renderCharts() {
  ...
  if (canvasTrend) {
    ...
    if (appState.calculator.isBaselineCalculated) {
      appState.history[appState.history.length - 1].emissions = parseFloat(
        appState.calculator.footprints.total.toFixed(1),
      );
    }
...
function toggleHabitCompletion(habit, isChecked) {
  ...
  if (isChecked) {
    ...
    if (checkedIndex !== -1) {
      ...
      appState.user.totalSavedCo2 = Math.max(
        0.0,
        appState.user.totalSavedCo2 - habit.co2Save,
      );
  ...
  const habitRow = document.querySelector(
    `.habit-item[data-habit-id="${habit.id}"]`,
  );
  if (habitRow) {
...
if (addCustomBtn) {
  addCustomBtn.addEventListener("click", () => {
    ...
    if (!inputName || !inputName.value.trim()) {
      triggerNotification(
        "⚠️ Please enter a description for your custom action.",
        "info",
      );
      return;
    }
    ...
    renderHabitsChecklist();
    triggerNotification(
      `✅ Successfully logged action: "${name}"`,
      "check-circle",
    );
  });
}
...
function updateImpactMetrics() {
  ...
  const todayEquivalentBanner = document.getElementById(
    "today-equivalent-banner",
  );
  ...
  if (todayEquivalentBanner) {
    if (totalSaved === 0) {
      todayEquivalentBanner.innerHTML =
        "<p>Start checking habits to see your carbon offsets visualized!</p>";
    } else {
...
function removeTimelineEvent(name) {
  ...
  const item = container.querySelector(
    `.timeline-item[data-timeline-ref="${name}"]`,
  );
  if (item) {
  ...
  if (
    ...
    (container.children.length === 1 &&
      container.querySelector(".timeline-empty"))
  ) {
    const emptyMsg = container.querySelector(".timeline-empty");
  ...
  appState.challenges.forEach((chal) => {
    ...
    if (chal.isJoined || chal.isCompleted) {
      ...
      joinBtn.addEventListener("click", () => {
        ...
        renderChallengesAndBadges();
        triggerNotification(
          `Joined challenge: "${chal.title}". Get to work!`,
          "award",
        );
      });
...
function evaluateManualProgress(chal) {
  ...
  if (chal.id === "solar-sovereign") {
    ...
    if (solar >= chal.targetVal) {
      ...
      triggerNotification(
        `Solar percentage is currently ${solar}%. Set renewable mix above 50% in the calculator.`,
        "info",
      );
    }
  } else {
    // Evaluate other logs
    triggerNotification(
      `Keep checking items off in your Daily habits tab! Progress is ${chal.progress}/${chal.target}.`,
      "info",
    );
  }
}
...
function completeChallenge(chal) {
  ...
  triggerNotification(
    `🏆 Challenge Complete: "${chal.title}" unlocked! Earned +${chal.xpReward} XP and a new badge!`,
    "award",
  );
...
function initEcoHub() {
  ...
  const slidersList = [
    {
      el: commuteSlider,
      label: "swap-val-commute",
      saved: "swap-saved-commute",
      unit: " km",
      factor: 10.4,
    }, // km * 52 * 0.2
    {
      el: meatSlider,
      label: "swap-val-meat",
      saved: "swap-saved-meat",
      unit: " meals",
      factor: 78.0,
    }, // meal * 52 * 1.5
    {
      el: flightSlider,
      label: "swap-val-flight",
      saved: "swap-saved-flight",
      unit: " flights",
      factor: 450.0,
    }, // flight * 450
    {
      el: energySlider,
      label: "swap-val-energy",
      saved: "swap-saved-energy",
      unit: "%",
      factor: 12.6,
...
function calculateSwapSimulatorTotal() {
  const commuteVal =
    parseFloat(document.getElementById("swap-slider-commute").value) || 0;
  const meatVal =
    parseFloat(document.getElementById("swap-slider-meat").value) || 0;
  const flightVal =
    parseFloat(document.getElementById("swap-slider-flight").value) || 0;
  const energyVal =
    parseFloat(document.getElementById("swap-slider-energy").value) || 0;
...
function loadQuizQuestion() {
  ...
  document.getElementById("quiz-question-num").textContent =
    `Question ${quiz.currentQuestionIndex + 1} of 5`;
  document.getElementById("quiz-points-accumulated").textContent =
    `Score: ${quiz.score} / ${quiz.currentQuestionIndex}`;
...
function showQuizResults() {
  ...
  if (appState.quiz.score === 5) {
    if (!appState.unlockedBadges.includes("badge-quiz")) {
      ...
      triggerNotification(
        "🏆 Perfect Score! Unlocked the 'Quiz Master' profile badge!",
        "award",
      );
      launchCelebrationConfetti(3);
    }
...
function exportReportAsPDF() {
  ...
  triggerNotification(
    "📄 Opening print dialog. Tip: Set destination to 'Save as PDF'.",
    "file-text",
  );
